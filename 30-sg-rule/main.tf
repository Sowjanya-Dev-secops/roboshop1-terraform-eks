...
resource "aws_security_group_rule" "eks_controll_plane_bastion" {
  type              = "ingress"
...
# eks node can accept all kinf of traffic from eks_controol_plane
resource "aws_security_group_rule" "eks_node_eks_controll_plane" {
  type              = "ingress"
  security_group_id = local.eks_node_sg_id
  source_security_group_id = local.eks_controll_plane_sg_id
  from_port         = 0
  protocol          = "-1"
  to_port           = 0
}
resource "aws_security_group_rule" "eks_controll_plane_eks_node" {
  type              = "ingress"
  security_group_id = local.eks_controll_plane_sg_id
...
# this is mandatory for pod to pod communcation. because pods can be any node in VPC CIDR
